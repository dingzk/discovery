...
#define SEM_FILENAME_PREFIX ".ZendSem_v2."
...
// memory model
static const char *memory_model = NULL;
...
int zend_shared_alloc_startup(size_t requested_size)
{
...
#ifndef ZEND_WIN32
	zend_shared_alloc_create_lock("/tmp");
#else
	zend_shared_alloc_create_lock();
#endif

	if (memory_model) {
		const char *model = memory_model;
		/* "cgi" is really "shm"... */
		if (strncmp(memory_model, "cgi", sizeof("cgi")) == 0) {
			model = "shm";
		}
	...
	/* move shared_segments and shared_globals to shared memory */
